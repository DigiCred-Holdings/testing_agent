...
import type { BasicMessageStateChangedEvent, InitConfig } from '@credo-ts/core'
import { Agent, AutoAcceptCredential, BasicMessageEventTypes, BasicMessageRole, ConnectionsModule, CredentialsModule, DidsModule, MediationRecipientModule, MediatorPickupStrategy, V2CredentialProtocol } from '@credo-ts/core'
import { agentDependencies } from '@credo-ts/node'
import { AskarModule } from '@credo-ts/askar'
import { ariesAskar } from '@hyperledger/aries-askar-nodejs'
import { anoncreds } from '@hyperledger/anoncreds-nodejs'
import { AnonCredsCredentialFormatService, AnonCredsModule, DataIntegrityCredentialFormatService, LegacyIndyCredentialFormatService, V1CredentialProtocol } from '@credo-ts/anoncreds'
...
import { HttpOutboundTransport, WsOutboundTransport, CredentialEventTypes, CredentialState, CredentialStateChangedEvent } from '@credo-ts/core'
...
export class AgentService {
    ...
    async agentInit(agentInitDto: AgentInitDto): Promise<string> {
        ...
        try {
            ...
            await this.setupCredentialListener(agentInitDto.agentName);
            await this.setupMessageListener(agentInitDto.agentName);
    ...
    async setupCredentialListener(agentName: string) {
        console.log("*** Connection Service: setupCredentialListener");
        const agent: Agent = await this.getAgentByName(agentName);
        console.log("Agent name=", agentName)
        agent.events.on(CredentialEventTypes.CredentialStateChanged, async ({ payload }: CredentialStateChangedEvent) => {
            console.log('=== Credential Listener');
            switch (payload.credentialRecord.state) {
              case CredentialState.OfferReceived:
                console.log('=== Offer Received state=', payload.credentialRecord.state);
                // custom logic here
                await agent.credentials.acceptOffer({ credentialRecordId: payload.credentialRecord.id })
                break;
              case CredentialState.Done:
                console.log('=== Done - Credential for credential id ',payload.credentialRecord.id, ' is accepted')
    ...
    async setupMessageListener(agentName: string) {
        console.log("*** Connection Service: setupMessageListener");
        const agent: Agent = await this.getAgentByName(agentName);
        console.log("Agent name=", agentName)
